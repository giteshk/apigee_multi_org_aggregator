...
foreach($_REQUEST['orgs'] as $org) {
    $time = time();
    $curl = curl_init( 'https://api.enterprise.apigee.com/v1/o/' . $org . '/developers?expand=true');
  $headers = [
  ...
  curl_setopt_array($curl, $options);
    ...
    $developer_stats[$org] = ['count' => count($developers->developers), 'total_time' => $time_elapsed_secs, 'timestamp' => $time];
}
...
foreach($developer_stats as $org => $info) {
    $query = "INSERT INTO analytics values('{$org}', 'all_developers_expanded', {$info['timestamp']}, '*', '', {$info['total_time']}, {$info['count']})";
    $conn->query($query);
}
